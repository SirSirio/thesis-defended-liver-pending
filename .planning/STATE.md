...
status: complete
stopped_at: v21 live. The pick button was broken by openPicker() recursion and is fixed; the re-pick cache is keyed on name+size, not lastModified; the Files door persists per device. Android still awaits a first-pick retest.
last_updated: "2026-08-29T07:30:00.000Z"
last_activity: 2026-08-29
last_activity_desc: v21 fix. openPicker() self-recursion (the dead pick button) fixed; byte cache re-keyed name+size so a re-pick survives the phone's changing lastModified; Files door persisted via store altpicker; reader ladder cut to 15s. Verified 8 cases in real Chrome + 3 pages/2 viewports, deploy confirmed on the wire.
progress:
  total_phases: 4
  completed_phases: 3
  total_plans: 19
...
| Asset versions | `?v=21` on every asset in both pages, and in `check.html` (bumped 2026-08-29 for the v21 fix). **Bump this on every deploy** or phones serve stale files for ten minutes. `index.html` itself carries no version and is cached 600s by GitHub Pages, so a hard refresh is needed to see a new deploy. Verified live 2026-08-29: `app.js?v=21` serves the fixed `openPicker`, `index.html` references `?v=21`. |
