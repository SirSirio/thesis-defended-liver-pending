...
| Asset versions | `?v=23` on every asset in all pages including `check.html` and `admin.html` (bumped 2026-08-31). **Bump this on every deploy** or phones serve stale files for ten minutes. `index.html` itself carries no version and is cached 600s by GitHub Pages, so a hard refresh is needed to see a new deploy. |
