...
last_activity_desc: Quick 260829-mem: the upload pipeline rebuilt around one read into memory, then never the File again
progress:
  total_phases: 4
  completed_phases: 3
  total_plans: 19
...
| Asset versions | `?v=18` on every asset in both pages, and in `check.html`. **Bump this on every deploy** or phones serve stale files for ten minutes. `index.html` itself carries no version and is cached 600s by GitHub Pages, so a hard refresh is needed to see a new deploy. |
