...
status: complete
stopped_at: Quick 260828-hef complete and live; the Android refusal itself still awaits a check.html report from that phone
last_updated: "2026-08-28T21:40:00.000Z"
last_activity: 2026-08-28
last_activity_desc: Quick 260828-hef: HEIC decoded by the site itself; any picture a phone makes now uploads
progress:
  total_phases: 4
  completed_phases: 3
  total_plans: 19
...
| Asset versions | `?v=15` on every asset in both pages, and in `check.html`. **Bump this on every deploy** or phones serve stale files for ten minutes. `index.html` itself carries no version and is cached 600s by GitHub Pages, so a hard refresh is needed to see a new deploy. |
