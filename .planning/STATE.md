...
last_activity_desc: Quick 260829-mem plus the re-pick cache and the Files door; the beacon now says which picker a refusal came through
progress:
  total_phases: 4
  completed_phases: 3
  total_plans: 19
...
| Asset versions | `?v=20` on every asset in both pages, and in `check.html`. **Bump this on every deploy** or phones serve stale files for ten minutes. `index.html` itself carries no version and is cached 600s by GitHub Pages, so a hard refresh is needed to see a new deploy. |
