...
status: complete
stopped_at: v25 live. Topbar upload pill (rides photosOpen(), :has() makes room in the bar) plus qr.html, the printable A4 QR poster (colour and black-and-white, QR verified decoding with jsQR). Android still awaits a first-pick retest (force the portal open from admin.html to test before the day).
last_updated: "2026-08-31T13:05:00.000Z"
last_activity: 2026-08-31
last_activity_desc: v25 upload pill in the topbar + qr.html poster. Pill proved in real Chrome at 1280 and 390 in all three languages; poster QR decoded from a screenshot back to the live URL.
progress:
  total_phases: 4
  completed_phases: 3
  total_plans: 19
...
| Asset versions | `?v=25` on every asset in all pages including `check.html` and `admin.html` (bumped 2026-08-31, the upload pill and the QR poster). **Bump this on every deploy** or phones serve stale files for ten minutes. `index.html` itself carries no version and is cached 600s by GitHub Pages, so a hard refresh is needed to see a new deploy. |
...
| THE REGISTER UNDER THE PIN (2026-08-31) | admin.html gained a register section: `admin_list_enrollments` / `admin_edit_enrollment` / `admin_delete_enrollment` (schema section 14), all PIN-checked, rows addressed by enrollment `id` and never guest_id. A name edit also rewrites `photos.name` for that guest so album attribution follows (proved: ZZAAA to ZZBBB on the wire). A delete cannot reach the guest's device: their phone keeps its receipt and re-registers them (same guest_id) the next time they touch the form; their photographs stay. **The PIN now also guards the private notes** - accepted trade, owner's short PIN. |
