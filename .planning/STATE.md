...
status: complete
stopped_at: Quick 260828-hef addendum 2 live; refusals now file themselves in public.diagnostics
last_updated: "2026-08-28T22:30:00.000Z"
last_activity: 2026-08-28
last_activity_desc: The Android refusals named by the device itself, NotReadableError from the content provider; reads retried, sentence added, diagnostics table live
progress:
  total_phases: 4
  completed_phases: 3
  total_plans: 19
...
| Asset versions | `?v=17` on every asset in both pages, and in `check.html`. **Bump this on every deploy** or phones serve stale files for ten minutes. `index.html` itself carries no version and is cached 600s by GitHub Pages, so a hard refresh is needed to see a new deploy. |
