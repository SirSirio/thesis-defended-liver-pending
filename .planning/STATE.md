...
status: complete
stopped_at: Quick 260829-mem live; the Android awaits a retest, and public.diagnostics now records which reader produced the bytes
last_updated: "2026-08-29T06:20:00.000Z"
last_activity: 2026-08-29
last_activity_desc: Quick 260829-mem plus the re-pick cache; a removed file picked again lands from memory
progress:
  total_phases: 4
  completed_phases: 3
  total_plans: 19
...
| Asset versions | `?v=19` on every asset in both pages, and in `check.html`. **Bump this on every deploy** or phones serve stale files for ten minutes. `index.html` itself carries no version and is cached 600s by GitHub Pages, so a hard refresh is needed to see a new deploy. |
